(module
    ...
    (func $get_x (import "ono" "get_x") (result i32))
    (func $get_y (import "ono" "get_y") (result i32))
    (func $get_x_prime (import "ono" "get_x_prime") (result i32))
    (func $get_y_prime (import "ono" "get_y_prime") (result i32))
    (func $get_n (import "ono" "get_n") (result i32))
  ...
  ;; Gros match permettant de selectionner la bonne contrainte
  (func $select_constraint (param $num_constraint i32) (result i32)
    (if (i32.eq (local.get $num_constraint) (i32.const 0))
      (then (return (call $is_alive (call $get_x) (call $get_y))))
    )
    (if (i32.eq (local.get $num_constraint) (i32.const 1))
      (then (return (call $is_dead (call $get_x) (call $get_y))))
    )
    (if (i32.eq (local.get $num_constraint) (i32.const 2))
      (then (return (call $at_least_one_alive)))
    )
    (if (i32.eq (local.get $num_constraint) (i32.const 3))
      (then (return (call $all_alive)))
    )
    (if (i32.eq (local.get $num_constraint) (i32.const 4))
      (then (return (call $all_dead)))
    )
    (if (i32.eq (local.get $num_constraint) (i32.const 5))
      (then (return (call $full_line (call $get_x) (call $get_y) (call $get_x_prime))))
    )
    (if (i32.eq (local.get $num_constraint) (i32.const 6))
      (then (return (call $full_column (call $get_x) (call $get_y) (call $get_y_prime))))
    )
    (if (i32.eq (local.get $num_constraint) (i32.const 7))
      (then (return (call $exactly_n_alive (call $get_n))))
    )
    (if (i32.eq (local.get $num_constraint) (i32.const 8))
      (then (return (call $has_isolated_cell)))
    )
    (if (i32.eq (local.get $num_constraint) (i32.const 9))
      (then (return (call $has_surrounded_cell)))
    )
    (if (i32.eq (local.get $num_constraint) (i32.const 10))
      (then (return (call $has_two_adjacent_alive)))
    )
    (if (i32.eq (local.get $num_constraint) (i32.const 11))
      (then (return (call $has_l_pattern)))
    )
    (if (i32.eq (local.get $num_constraint) (i32.const 12))
      (then (return (call $has_square_pattern)))
    )
    (if (i32.eq (local.get $num_constraint) (i32.const 13))
      (then (return (call $dead_to_alive_transition)))
    )
    (if (i32.eq (local.get $num_constraint) (i32.const 14))
      (then (return (call $has_alternating_pattern)))
    )
    (if (i32.eq (local.get $num_constraint) (i32.const 15))
      (then (return (call $has_blinker_pattern)))
    )
    (if (i32.eq (local.get $num_constraint) (i32.const 16))
      (then (return (call $has_diagonal_n (call $get_n))))
    )
    (i32.const 0)
  )
    ...
    (func $main
        ...
        ;;contraintes ()
        (if (call $select_constraint (local.get $num_constraint))
          (then unreachable)
          (else return )
        )
