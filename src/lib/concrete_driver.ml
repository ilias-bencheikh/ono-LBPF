...
let run ~source_file ~max_steps ~use_graphics =
  (* Storage pour max_steps *)
  if use_graphics then Concrete_ono_module_raylib.set_max_steps max_steps
  else Concrete_ono_module.set_max_steps max_steps;
